...
export default function NewNote() {
  ...
  return (
    <Form method="post">
      ...
      <FlexList pad={4}>
        <p>Are you sure you want to delete this note? You cannot get it back once its deleted.</p>
      </FlexList>
      <div className="bg-base-100 shadow-2xl flex flex-col p-4 gap-2 w-full sm:flex-row-reverse xl:rounded-md">
        <Button size="md" type="submit" kind="error" icon={faTrash} isSaving={isSubmitting}>{isSubmitting ? 'Deleting...' : 'Delete note'}</Button>
        <Link to="..">Cancel</Link>
      </div>
    </Form>
  )

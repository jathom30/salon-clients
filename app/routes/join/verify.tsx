...
export async function action({ request }: ActionArgs) {
  ...
  const token = urlSearchParams.get('token')
  const id = urlSearchParams.get('id')
  const redirectTo = safeRedirect(urlSearchParams.get("redirectTo"), "/clients");
  invariant(id, 'User id not found')
  invariant(token, 'Token not found')
  const user = await getUserById(id)
      ...
      <FlexList pad={4}>
        ...
        <ItemBox>
          <FlexList>
            ...
            <p>If you'd like to request a new email, <Link className="link link-primary" to="/join/requestVerification">click here</Link>.</p>
          </FlexList>
        </ItemBox>
      </FlexList>
    </div>
